// Assignment 3.1 Loops.cpp : This file contains the 'main' function. Program execution begins and ends there.
// 2/3/2025
// Adding a do loop to Jumanji code
    ...
    do {
        ...
        if (firstChoice == 1) {
            ...
            cout << "You shout for Nigel but your voice attracts a wild boar instead! You try escaping but end up being killed by the wild boar" <<
                endl << "GAME OVER";
            // After death player is asked if they want to play again
            cout << "\nDo you want to play again? (Y/N): ";
            // This takes user input for the do while loop to work properly
            cin >> restartChoice;
            continue; // Continue; allows game to restart to the very beginning
        }
...
// three differnt paths to take
    ...
    // After multiple attempts of using (restartChoice == 'Y' || restartChoice == 'n')
    // Figured out it was the wrong one to use so with the (restartChoice != 'N' && restartChoice != 'n')
    // Code runs properly
